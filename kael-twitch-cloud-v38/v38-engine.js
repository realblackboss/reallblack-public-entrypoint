...
const EGRESS_API='https://qfkcripudkcmsdxrhdbt.supabase.co/functions/v1/kael-v38-egress';
...
let snap=null, selected=null, adminKey='', volume=0, quality='auto', deviceEgressIp='IP PENDENTE', egressSource='';
...
function tenantEgressIp(t){
  return String(t?.node_ip||t?.egress_ip||deviceEgressIp||t?.masked_ip||'IP PENDENTE');
}
function refreshEgressVisuals(){
  for(const [id,c] of cards){
    const t=tenantMap.get(id);
    if(!t)continue;
    const ip=tenantEgressIp(t);
    setText(c.title,'@'+(t.target_channel??'')+' | '+(t.display_name??'')+' | '+(t.network_mode??'TWITCH_EMBED_DIRECT')+' | '+ip);
    setText(c.route,(t.egress_isolated?'NODE_EGRESS':'DEVICE_EGRESS')+' | '+ip);
    c.route.className='route good';
  }
  if(snap)updateMetrics(snap);
}
async function loadEgress(){
  try{
    const r=await fetch(EGRESS_API,{cache:'no-store'});
    if(!r.ok)throw Error('HTTP '+r.status);
    const j=await r.json();
    if(j?.ip){
      deviceEgressIp=String(j.ip);
      egressSource=String(j.source||'edge-request');
      refreshEgressVisuals();
      return deviceEgressIp;
    }
  }catch(e){
    console.warn('V38 egress detect',e);
  }
  deviceEgressIp='IP INDISPONÍVEL';
  refreshEgressVisuals();
  return null;
}
...
function updateMetrics(s){
  ...
  setText($('summary'),total+' perfis'+lotInfo+' | '+playing+'/'+players.size+' vídeos tocando | '+ok+' login OK | '+pending+' login pendente | '+no+' sem login | egress '+deviceEgressIp+' | vídeo Twitch → '+location.hostname);
}
...
function filterCards(){
  ...
  for(const [id,c] of cards){
    const t=tenantMap.get(id);
    const hay=((t?.display_name||'')+' '+(t?.target_channel||'')+' '+tenantEgressIp(t)+' '+(t?.network_mode||'')).toLowerCase();
